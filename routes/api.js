...
router.get('/jadwal-sholat/2/:type', async (req, res, next) => {
  try {
    const type = req.params.type,
      text = req.query.text,
...
router.get('/islam/jadwal-sholat', async (req, res, next) => {
  try {
    var nomer = req.query.nomer;
    var nomer = nomer ? nomer : '1'
    var nomer = Number(nomer)
    if (isNaN(nomer)) return res.json({ developer: develop, mess: 'Input number, not a string!' })
